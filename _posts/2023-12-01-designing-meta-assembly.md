...
- We have
  ...
  ```
Which is problematic, as we have dependent return type (dependent argument can be changed into dependent return via currying). But we know that it is `Dep.DepA` instance. It brings me to a conclusion, that `Dep.DepA` must be expressed via erased generics, so return type is actually union `{ DepA : Config IdisAny }`

<br />

As such, all used variants can be expressed
